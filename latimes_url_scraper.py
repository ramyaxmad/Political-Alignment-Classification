...
def get_latimes_politics_article_links():
    ...
    for a_tag in soup.find_all('a', href=True):
        ...
        if '/202' in full_url and '/politics/' in full_url:
            all_links.add(full_url)

    return list(all_links)

def save_links_to_csv(links, filename='latimes_articles.csv'):
...
if __name__ == '__main__':
    all_links = get_latimes_politics_article_links()
    if not all_links:
        print("links gotten!")
    else:
