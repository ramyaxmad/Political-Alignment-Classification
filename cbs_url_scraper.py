...
def get_cbs_political_links():
    ...
    for a_tag in soup.find_all('a', href=True):
        ...
        if href.startswith('https://www.cbsnews.com/') and any(kw in href for kw in political_keywords):
            article_links.add(href.split('?')[0])

    return list(article_links)

def save_links_to_csv(links, filename='cbs_articles.csv'):
...
if __name__ == '__main__':
    all_links = get_cbs_political_links()
    if not all_links:
        print("None found, yo")
    else:
